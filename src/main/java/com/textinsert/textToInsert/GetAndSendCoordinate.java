package com.textinsert.textToInsert;

//import lombok.SneakyThrows;
import org.apache.pdfbox.pdmodel.PDDocument;
import org.apache.pdfbox.text.PDFTextStripper;
import org.apache.pdfbox.text.TextPosition;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.beans.factory.annotation.Qualifier;
import org.springframework.stereotype.Component;

import java.io.*;
import java.util.List;

//@Component
public class GetAndSendCoordinate extends PDFTextStripper {

    public static String textToFind = "Уникальный номер записи об аккредитации в реестре аккредитованных лиц ________";
    public static String textToFind1 = "должность руководителя или";
    public static String textToFind2 = "фамилия, инициалы";
    public static String forText = "RA.RU.312849";
    public static String forText1 = "metrolog";
    public static String forText2 = "FIO";

    public GetAndSendCoordinate() throws IOException {
            super();
    }

    private String stringForFile;

    //@Autowired
    public GetAndSendCoordinate(String stringForFile) throws IOException {
        this.stringForFile = stringForFile;

    }

    //@Override
    public void runer() {
        try {
            //String newString = CollectStrings.createStringFilename();
            System.out.println("stringForFile:  " + stringForFile);
            File file = new File(stringForFile);
            PDDocument document = PDDocument.load(file);
            PDFTextStripper stripper = new GetAndSendCoordinate();
            Writer dummy = new OutputStreamWriter(new ByteArrayOutputStream());
            stripper.writeText(document, dummy);
            document.save(stringForFile);
            document.close();
            //WorkWithFile.recieveFile(stringForFile);


        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    //String string = "agddshdshsg";
    //@SneakyThrows
    @Override
    protected void writeString(String string, List<TextPosition> textPositions) throws IOException {
        //super.writeString(string);
        //System.out.printf("string" + string);
        if (string.equalsIgnoreCase(textToFind)) {
            TextPosition textPosition = textPositions.get(string.length() - 7);
            float xDirAdj = textPosition.getXDirAdj();
            float yEndAdj = textPosition.getEndY();

            InsertCoordinatesToDB.InsertValueToTable(stringForFile, textToFind, xDirAdj, yEndAdj, forText);
        }

        if (string.equalsIgnoreCase(textToFind1)) {
            TextPosition textPosition = textPositions.get(0);
            float xDirAdj1 = textPosition.getXDirAdj();
            float yEndAdj1 = textPosition.getEndY() + 12;

            InsertCoordinatesToDB.InsertValueToTableNext(stringForFile, textToFind1, xDirAdj1, yEndAdj1, forText1);
        }

        if (string.equalsIgnoreCase(textToFind2)) {
            TextPosition textPosition = textPositions.get(0);
            float xDirAdj2 = textPosition.getXDirAdj();
            float yEndAdj2 = textPosition.getEndY() + 12;

            InsertCoordinatesToDB.InsertValueToTableNext(stringForFile, textToFind2, xDirAdj2, yEndAdj2, forText2);
        }

    }

}

